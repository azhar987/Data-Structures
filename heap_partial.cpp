...
bool Heap::isMinHeap(int arr[], int i, int n)
{
	int left= 2*i+1;
	int right= 2*i+2;
    ...
    if (arr[i] >= arr[left] && arr[i] >= arr[right] && isMinHeap(arr, left, n) && isMinHeap(arr, right, n))
        return true;
...
int main()
{
    ...
    h2.print();
	int n = arr_sz - 1;
    h2.isMinHeap(arr, 0, n) ? cout<<"Yes, its a heap." : cout<<"No, its not a heap";
    return 0;
}
